...
<div class="my-campaign wfp-content-padding">
	...
	<div class="campaign-tab-container">	
		...
		if ( isset( $_GET['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['nonce'] ) ), 'wp_rest' ) ) {
			$wfpGetType = isset( $_GET['type'] ) ? sanitize_text_field( wp_unslash( $_GET['type'] ) ) : 'all';
		} else {
			$wfpGetType = 'all';
		}
		...
		<div class="campaign-body">
			...
			if ( $wfpGetType == 'all' ) {
				$wfp_args['post_status'] = array( 'draft', 'publish', 'pending' );
			} elseif ( $wfpGetType == 'review' ) {
				$wfp_args['post_status'] = 'pending';
			} else {
				$wfp_args['post_status'] = ( $wfpGetType == 'publish' ) ? 'publish' : 'draft';
			}
