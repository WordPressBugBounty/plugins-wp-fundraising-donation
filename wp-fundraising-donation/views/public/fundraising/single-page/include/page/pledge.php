		...
		<div class="wfp-pledge-block" id="wfp-pledge-block__<?php echo esc_attr( $wfp_m ); ?>" onclick="wfp_select_pledge(this);">
			...
			<div class="pledge__detail">
				...
				<span class="pledge__detail-info">
					...
					echo esc_html( WfpFundraising\Apps\Settings::wfp_number_format_currency_icon( 'left', $wfp_defaultUse_space ) );
					?>
					<strong><?php echo esc_html( WfpFundraising\Apps\Settings::wfp_number_format_currency( $wfp_amount ) ); ?></strong>
			...
			<?php
			$wfpQuantityData = isset( $multi->quantity ) ? $multi->quantity : '';

			if ( $wfpQuantityData > 0 ) {

				if ( empty( $multi->id ) || empty( $wfp_opt[ $wfpPostId ][ $multi->id ] ) ) {
					$wfp_claimed = 0;
				} else {
					$wfp_claimed = empty( $wfp_opt[ $wfpPostId ][ $multi->id ] ) ? 0 : count( $wfp_opt[ $wfpPostId ][ $multi->id ] );
				}
			...
			$wfpIncludesData = isset( $multi->includes ) ? $multi->includes : '';
			if ( strlen( $wfpIncludesData ) > 2 ) {
				$wfpExplodeLi = explode( ',', $wfpIncludesData );
				?>
			...
			$wfpEstimatedData = isset( $multi->estimated ) ? $multi->estimated : '';
			...
			$wfpShipsdData = isset( $multi->ships ) ? $multi->ships : '';
				...
				if ( $wfp_campaign_status == 'Publish' ) {
					...
					<div class="wfp-additional-data">
						<div class="wfdp-input-payment-field xs-fixed-lebel">
							<div class="pledge__detail-info">
								...
								<input type="number"
									   ...
									   disabled
									   value="<?php echo esc_attr( strlen( $wfp_amount ) > 0 ? $wfp_amount : 0 ); ?>"
									   placeholder="1.00"
						...
						<button type="submit"
								...
								data-pledge_amt="<?php echo esc_attr( empty( $wfp_amount ) ? '0' : floatval( $wfp_amount ) ); ?>"
								data-gateway="<?php echo esc_attr( \WfpFundraising\Apps\Global_Settings::instance()->get_payment_type() ); ?>"
								data-wfp-index="<?php echo esc_attr( $wfp_m ); ?>"
								wfp-id="<?php echo esc_attr( $wfpPostId ); ?>"
								wfp-index="<?php echo esc_attr( $wfp_m ); ?>"
								data-checkout_url="<?php echo esc_url( add_query_arg( 'wpf_checkout_nonce_field', wp_create_nonce( 'wpf_checkout' ), $wfpUrlCheckout ) ); ?>"
